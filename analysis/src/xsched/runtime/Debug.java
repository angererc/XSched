...
import xsched.runtime.Sched;
import xsched.runtime.ThreadPool.WorkItem;
...
class Debug {
	...
	static class NewActivationEvent extends Event {
		public final Sched.WithThis<?,?> activation;

		public NewActivationEvent(Sched.WithThis<?,?> inter) {
	...
	public static void newActivation(Sched.WithThis<?,?> activation) {
		if(ENABLED_SCHEDULE)
	...
	static class ActivationStateChangeEvent extends Event {
		...
		public ActivationStateChangeEvent(Sched.WithThis<?,?> inter) {
			this.activation = inter;
			this.retainCount = ((Sched<?,?>)inter).retainCount();
			if(inter.isInFuture()) {
	...
	public static void activationStateChange(Sched.WithThis<?,?> activation) {
		if(ENABLED_SCHEDULE)
