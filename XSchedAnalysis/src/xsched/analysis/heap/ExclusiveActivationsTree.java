...
public class ExclusiveActivationsTree<Context> extends ActivationsTreeInnerNode<Context> {

	public ExclusiveActivationsTree(Context context) {
		super(context);
	}
			...
			for(int j = i+1; j < size; j++) {
				for(CreationEdge<Context> first : creationEdgesLists.get(i)) {
					for(CreationEdge<Context> second : creationEdgesLists.get(j)) {
						first.isExclusiveTo(second);
						second.isExclusiveTo(first);
					}
				}
			}
		}
